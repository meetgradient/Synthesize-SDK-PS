Function InitGradientConnection {
    ...
    #Validate ENV
    if ([string]::IsNullOrWhiteSpace($Env:CLIENT_ID) -OR [string]::IsNullOrWhiteSpace($Env:CLIENT_SECRET) -OR [string]::IsNullOrWhiteSpace($Env:BASE_URL) -OR [string]::IsNullOrWhiteSpace($Env:SUBSCRIPTION_KEY)) {
    Throw "Sherweb API environment variables are required in ENV"
    }
