Function InitGradientConnection {
    ...
    #Validate ENV
    if ([string]::IsNullOrWhiteSpace($Env:BASE_URL) -OR [string]::IsNullOrWhiteSpace($Env:CLIENT_ID) -OR [string]::IsNullOrWhiteSpace($Env:CLIENT_SECRET)) {
    Throw "Addigy API environment variables are required in ENV"
    }
