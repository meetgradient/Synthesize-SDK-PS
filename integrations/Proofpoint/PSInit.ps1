Function InitGradientConnection {
    ...
    #Validate ENV
    if ([string]::IsNullOrWhiteSpace($Env:USERNAME) -OR [string]::IsNullOrWhiteSpace($Env:PASSWORD) -OR [string]::IsNullOrWhiteSpace($Env:BASE_URL) -OR [string]::IsNullOrWhiteSpace($Env:ORGANIZATION_DOMAIN)) {
    Throw "Proofpoint API environment variables are required in ENV"
    }
