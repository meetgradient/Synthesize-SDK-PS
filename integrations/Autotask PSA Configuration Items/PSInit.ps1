Function InitGradientConnection {
    ...
    #Validate ENV
    if ([string]::IsNullOrWhiteSpace($Env:USERNAME) -OR [string]::IsNullOrWhiteSpace($Env:PASSWORD) -OR [string]::IsNullOrWhiteSpace($Env:INTEGRATION_CODE)) {
    Throw "Autotask PSA API environment variables are required in ENV"
    }
