Function InitGradientConnection {
    Param(
        ...
        # Validate usecure env
        If ([string]::IsNullOrWhiteSpace($Env:API_URL) -AND [string]::IsNullOrWhiteSpace($Env:API_KEY) ) {
            Throw 'usecure API key and URL are required in ENV'
        }
