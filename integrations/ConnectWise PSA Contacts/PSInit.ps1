Function InitGradientConnection {
    ...
    #Validate ENV
    if ([string]::IsNullOrWhiteSpace($Env:PUBLIC_KEY) -OR [string]::IsNullOrWhiteSpace($Env:PRIVATE_KEY) -OR [string]::IsNullOrWhiteSpace($Env:CLIENT_ID) -OR [string]::IsNullOrWhiteSpace($Env:CUSTOMER_ID)) {
    Throw "ConnectWise PSA API environment variables are required in ENV"
    }
